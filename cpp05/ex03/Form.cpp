...
/*   Updated: 2022/11/04 18:29:06 by jinacio-         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "Form.hpp"

Form::Form ( void ): _target("Marley"), _name("Pansa"), _signGra(5),
_execGra(5), _signed(false)
{
	return ;
}

Form::Form (std::string const name, int const signGra, int const execGra):
_name(name), _signGra(signGra), _execGra(execGra), _signed(false)
{
	return ;
}
...
const char *Form::WeNeedTheSign::what( void )const throw()
{
	return ("we need the signature  :/");
}
